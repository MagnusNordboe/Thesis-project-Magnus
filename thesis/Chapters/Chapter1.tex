...
The research area for this thesis is microservices and mahcine learning. An inevitable intersection appears between the two fields
...
the data is of acceptable quality. However, the field of machine learning applications on microservice system data is still immature as of time of writing. This thesis seeks to broach the subject further.\\
\keyword{Research question 1}: How good are current machine learning classification algorithms at identifying causes for anomalous behaviour in microservice systems?\\
...
\begin{itemize}
\item \keyword{Dynamic Time Warping}: A method of stasticically comparing time series across time points to judge similarity.
\item \keyword{Multivariate Time Series (MTS)}: Time series data with two or more variables.
\item \keyword{Instance}: All the data collected from a system in a specific time frame, represented as a MTS.
\item \keyword{Feature}: A unique Prometheus metric that expresses some kind of information about the system as a number. Collected at fixed intervals to form a MTS.
